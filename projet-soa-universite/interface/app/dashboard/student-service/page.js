...
export default function StudentService() {
    ...
    async function sendRequest() {
        try{
            ...
            if(method === "POST" || method==="PUT"){
                payload=JSON.parse(JSON.stringify(document.getElementById("json-format").value))
            }
            const res = await fetch("http://localhost:3000/api/student-service", {
                method: "POST",
    ...
    <fieldset className="auth-request">
        <legend>Manage Requests</legend>
            ...
            <TextField
            defaultValue="http://localhost:8080/students"
            size="small"
            fullWidth
            id="url"
            sx={{
            ...
            <div className="json-response-container">
                <h1>Response <span className={(status == 200|| status == 201)? "OK":(status==403||status==400)?"Forbidden":status==404?"not-found":""}>{status == 200? "OK"
                :status==403?"Forbidden":status==404?"Not Found":status==201?"Created":status==400?"Bad Request":""}</span></h1>
